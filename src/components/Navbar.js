				...
				<div className="flex items-center justify-between h-16">
					...
					<div className="hidden md:block">
						<div className="ml-10 flex items-baseline space-x-4">
							<Link
								href="/"
								className={`text-white hover:bg-gray-500 px-3 py-2 rounded-md text-sm font-medium ${activeClass(
                  "/"
								)}`}
							>
								Home
			...
			{isOpen && (
				<div className="md:hidden bg-gray-600">
					<div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
						<Link
							href="/"
							className={`text-white hover:bg-gray-500 block px-3 py-2 rounded-md text-base font-medium ${activeClass(
								"/"
							)}`}
						>
							Home
